...
# add in LHZ code here

```{R process livelihood data in R}
# read in LHZ Sensitivity data
# lhz_scores = read.csv(here(private_r, "lhz.csv"))
lhz_scores = read.csv(here("data", "raw", "private", "lhz.csv"))

#Join livelihood data into LHZ shp file

lhz_sensitivity = lhz_scores %>%
  left_join(lhz_scores, by = ("LZCODE" = "LZCODE")) %>%
  
  mutate(
    ownCrops = percent_rank(pctownCrops) * 4 + 1, 
    wageIncome = percent_rank(pctwageIncome) * 4 + 1, 
    cashCropIncome = percent_rank(desc(pctcashCropIncome)) * 4 + 1, 
    newDisaster = percent_rank(desc(pctnewDisaster)) * 4 + 1, 
  ) %>%
