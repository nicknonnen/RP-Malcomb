...
private_r = here("data","raw","private") #download data from local repository
...
```{R process livelihood data in R and right join to polygons}
# read in LHZ Sensitivity data
lhz_scores = read.csv(here(private_r "lhz.csv"))

#Join livelihood data into LHZ shp file
lhz_sensitivity <- lhz_scores %>% #problem here?
  right_join(lhz_scores, by = ("LZCODE" = "LZCODE")) %>%

# transform data into quintiles by percent rank, 0-1 scale
  mutate(
    ownCrops = percent_rank(pctownCrops) * 4 + 1,
    IncomeWage= percent_rank(desc(pctIncomeWage)) * 4 + 1,
    IncomeCashCrops = percent_rank(pctIncomeCashCrops) * 4 + 1,
...
ta = mutate(ta, capacity_2010 = capacity_2010 * 20) # 20% is the weight applied to AC
# 256 features 
...
fig4 <- st_read(dsn = paste0(here::here(), "/data/derived/public/georeferencing.gpkg"), layer="TAresilience_4")
fig5 <- st_read(dsn = paste0(here::here(), "/data/derived/public/georeferencing.gpkg"), layer="georef_vulnerability_grid")
```
